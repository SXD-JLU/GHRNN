...
def evaluate_loss(args, model, data, feature_map):
    ...
    x_t1, x_t2 = F.one_hot(t1, num_classes=max_nodes +
                           2)[:, :, :-1].float(), F.one_hot(t2, num_classes=max_nodes + 2)[:, :, :-1].float()
    x_v1, x_v2 = F.one_hot(v1, num_classes=len_node_vec +
                           1)[:, :, :-1].float(), F.one_hot(v2, num_classes=len_node_vec + 1)[:, :, :-1].float()
    x_e = F.one_hot(e, num_classes=len_edge_vec + 1)[:, :, :-1].float()
    ...
    if args.create_ghrnn == True:
        timestamp1, timestamp2, vertex1, edge, vertex2 = model['dfs_code_ghrnn'](dfscode_rnn_output,x_t1, x_t2, x_v1, x_e )
    else:
...
def predict_graphs(eval_args):
    ...
    for _ in range(eval_args.count // eval_args.batch_size):
        ...
        for i in range(eval_args.max_num_edges):
            ...
            # Evaluating dfscode tuple
            if train_args.create_ghrnn == True:
                timestamp1, timestamp2, vertex1, edge, vertex2 = model['dfs_code_ghrnn'](rnn_output)
                timestamp1 = timestamp1.reshape(eval_args.batch_size,-1)
                timestamp2 = timestamp2.reshape(eval_args.batch_size,-1)
                vertex1 = vertex1.reshape(eval_args.batch_size,-1)
                vertex2 = vertex2.reshape(eval_args.batch_size,-1)
                edge = edge.reshape(eval_args.batch_size,-1)
            else:
